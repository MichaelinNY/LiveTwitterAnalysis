...
module.exports = function() {
	...
	app.get("/start", function(req, res) {
		...
		if (track !== undefined) {
			var sql = "INSERT INTO \"LiveTwitterAnalysis.db::tables.Tracking\" (\"created\",\"name\",\"value\") VALUES(?,?,?)";
			...
			client.stream("statuses/filter", {
				...
				stream.on("data", function(data) {
					if (typeof data.id_str !== "undefined") {
						...
						var sql =
							"INSERT INTO \"LiveTwitterAnalysis.db::tables.Tweets\" (\"id\",\"created\",\"text\",\"lang\",\"user\",\"replyUser\",\"retweetedUser\") VALUES(?,?,?,?,?,?,?)";
						hdiClient.prepare(sql, function(err, statement) {
							if (err) {
								return console.log("PREPARE ERROR: " + err.toString());
							}
	...
	app.get("/stop", function(req, res) {
		var hdiClient = req.db;
		if (typeof client.currentTwitterStream !== "undefined") {
			var sql = "INSERT INTO \"LiveTwitterAnalysis.db::tables.Tracking\" (\"created\",\"name\") VALUES(?,?)";
			hdiClient.prepare(sql, function(err, statement) {
				if (err) {
					return console.log("PREPARE ERROR: " + err.toString());
				}
